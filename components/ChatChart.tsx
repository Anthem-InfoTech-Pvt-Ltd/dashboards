...
// ─── Types (matching all dashboard chart data structures) ─────────────────────
...
export interface ChatChartData {
    type: "pie" | "donut" | "bar" | "line" | "area" | "horizontalBar"
        | "daywise" | "tree" | "radar" | "stackedBar";
    title: string;
    currency: string;
    labels?: string[];
    datasets?: ChartDataset[];
    // daywise (YearlyCreditDebitChart format)
    creditSeries?: [number, number][];
    debitSeries?: [number, number][];
    // tree (ExpenseTreeChart format)
...
// ─── Exact colors from your existing charts ───────────────────────────────────

// HighLevelPieChart COLORS array (exact match)
const PIE_COLORS = ["#3b82f6", "#00b4d8", "#22c55e", "#60d394", "#f59e0b", "#f4a261", "#9d4edd", "#ff4d6d", "#ffc107", "#20c997"];

// StackedBarCategoryChart colors array (exact match)
const STACKED_COLORS = ["#ffc107", "#95c623", "#20c997", "#d00000", "#9d4edd", "#f77f00", "#4c9bfd", "#0dcaf0", "#ff4d6d", "#51291e"];

// ExpenseTreeChart colors (exact match from buildTree)
const TREE_COLORS = {
    root: "#e36414",       // isDark ? "#fb8b24" : "#e36414"
    yearNode: "#0077b6",   // isDark ? "#48cae4" : "#0077b6"  
    circle: "#6a994e",     // isDark ? "#a7c957" : "#6a994e"
    diamond: "#02c39a",    // isDark ? "#89c2d9" : "#02c39a"
};

// MonthlyRadarChart exact colors
const RADAR_CREDIT = "#60d394";
const RADAR_DEBIT = "#ef4444";

// YearlyCreditDebitChart exact colors
...
// ─── Highcharts loader (same pattern as your existing charts) ─────────────────
...
function loadHighcharts(cb: () => void) {
    ...
    // Load highstock first (like YearlyCreditDebitChart), then highcharts-more
...
// ─── ECharts loader (same pattern as ExpenseTreeChart) ────────────────────────
...
export default function ChatChart({ chartData, fullscreen = false }: ChatChartProps) {
    ...
    // Which library handles which chart (matching your existing components)
    const usesHighcharts = ["pie", "donut", "bar", "line", "area", "daywise", "horizontalBar", "stackedBar"].includes(type);
    const usesECharts = ["tree", "radar"].includes(type);

    // Heights matching your existing chart containers
    const chartHeight = (() => {
        if (type === "tree") {
        ...
        if (type === "radar") return fullscreen ? 420 : 360;        // MonthlyRadarChart h-[450px]
        if (type === "pie" || type === "donut") return fullscreen ? 340 : 300; // HighLevelPieChart h-[300px]
        if (type === "daywise") return fullscreen ? 400 : 340;      // YearlyCreditDebitChart h-[400px]
        ...
        return fullscreen ? 360 : 300; // bar/line/area — AreaYearlyExpenseChart h-[400px]
    })();
    ...
    useEffect(() => {
        ...
        // Destroy previous instance (same as your existing charts)
        try {
    ...
    // ── HIGHCHARTS — matching HighLevelPieChart, CategoryWiseExpenseChart, YearlyCreditDebitChart, StackedBarCategoryChart ──
    function renderHighcharts() {
        const el = containerRef.current!;
        const { labels = [], datasets = [] } = chartData;

        // ── PIE / DONUT — exact HighLevelPieChart style ──
        // HighLevelPieChart: type="pie", innerSize="50%", depth=45, borderRadius=5, borderWidth=3
        // COLORS: ["#3b82f6", "#00b4d8", "#22c55e", "#60d394", "#f59e0b", "#f4a261"]
            ...
            instanceRef.current = HC.chart(el, {
                ...
                plotOptions: {
                    pie: {
                        ...
                        cursor: "pointer",
                        depth: 45,
                        innerSize: type === "donut" ? "50%" : "50%", // HighLevelPieChart always 50%
                        showInLegend: true,
                        borderWidth: 3,
                        borderColor: "#ffffff",
                        dataLabels: {
                ...
                legend: {
                    ...
                    itemStyle: { color: "#444", fontWeight: "500", fontSize: "12px" },
                },
                series: [{
                    type: "pie",
                    name: "Expenses",
        ...
        // ── DAYWISE — exact YearlyCreditDebitChart style (Highstock stockChart + spline) ──
        // YearlyCreditDebitChart: constructorType="stockChart", Credit=#10B981, Debit=#EF4444
        if (type === "daywise") {
            ...
            instanceRef.current = HC.stockChart(el, {
                ...
                exporting: { enabled: false },
                accessibility: { enabled: false },
                // rangeSelector matching YearlyCreditDebitChart
                rangeSelector: {
                    selected: 1,
                    buttons: [
                    ...
                    buttonTheme: {
                        fill: "#f3f4f6",
                        style: { color: "#374151" },
                        states: { select: { fill: "#3b82f6", style: { color: "#fff" } } },
                    },
                },
                xAxis: {
                    type: "datetime",
                ...
                yAxis: {
                    ...
                    labels: {
                        formatter() { return fmt(this.value as number, currency); },
                        style: { color: "#374151" },
                    },
                ...
                tooltip: {
                    ...
                    // Matching YearlyCreditDebitChart pointFormatter
                    pointFormatter() {
                        return `<span style="color:${(this as any).color}">\u25CF</span> <b>${(this as any).series.name}: ${currency}${HC.numberFormat((this as any).y!, 0, ".", ",")}</b><br/>`;
                    },
                },
                plotOptions: {
                    spline: {
                ...
                legend: {
                    ...
                    align: "center",
                    verticalAlign: "bottom",
                    itemStyle: { color: "#111827", fontSize: "13px" },
                },
                series: [
                    {
                        name: "Credit",
                        data: creditSeries.map(([x, y]) => ({ x, y, marker: { enabled: y > 0 } })),
                        type: "spline",
                        color: DAYWISE_CREDIT, // #10B981
                        lineWidth: 2,
                    },
                    {
                        name: "Debit",
                        data: debitSeries.map(([x, y]) => ({ x, y, marker: { enabled: y > 0 } })),
                        type: "spline",
                        color: DAYWISE_DEBIT, // #EF4444
                        lineWidth: 2,
                    },
                ],
            });
        ...
        // ── BAR — exact AreaYearlyExpenseChart style (Highcharts column, Debit=#2563eb, Credit=#22c55e) ──
        if (type === "bar") {
            const seriesColors = ["#2563eb", "#22c55e"]; // AreaYearlyExpenseChart exact colors
            instanceRef.current = HC.chart(el, {
                accessibility: { enabled: false },
                ...
                tooltip: {
                    ...
                    pointFormatter() {
                        return `<span style="color:${(this as any).color}">\u25CF</span> ${(this as any).series.name}: <b>${fmt((this as any).y!, currency)}</b><br/>`;
                    },
                },
                legend: {
                ...
                plotOptions: {
                    column: { grouping: true, borderRadius: 4 },
                },
                series: datasets.map((ds, i) => ({
                    ...
                    borderWidth: 1,
                    borderRadius: 4,
                    tooltip: { valueFormatter: (v: number) => fmt(v, currency) },
                })),
            });
            return;
        }

        // ── LINE / AREA — CategoryWiseExpenseChart style (Highcharts spline, connectNulls, marker radius 4) ──
        if (type === "line" || type === "area") {
            // CategoryWiseExpenseChart: chart.type="spline", connectNulls=true, lineWidth=2, marker.radius=4
            // YearlyCreditDebitChart line colors: Debit=#ef4444, Credit=#22c55e
            ...
            instanceRef.current = HC.chart(el, {
                ...
                legend: {
                    ...
                    layout: "horizontal",
                    backgroundColor: "#ffffff",
                    itemStyle: { color: "#1f2937", fontWeight: "500", fontSize: "12px" },
                    borderColor: "#e5e7eb",
                    symbolWidth: 40,
                },
                tooltip: {
        ...
        // ── HORIZONTAL BAR — exact StackedBarCategoryChart style (Highcharts bar horizontal stacking) ──
        // StackedBarCategoryChart: chart.type='bar', stacking='normal', minPointLength=35
        // colors: ["#ffc107", "#95c623", "#20c997", "#d00000", "#9d4edd", "#f77f00", "#4c9bfd", "#0dcaf0", "#ff4d6d"]
        ...
        // ── STACKED BAR — StackedBarCategoryChart exact style (same as horizontal but vertical column) ──
        if (type === "stackedBar") {
            instanceRef.current = HC.chart(el, {
                accessibility: { enabled: false },
                chart: { type: "column", backgroundColor: "transparent", height: chartHeight },
                ...
                tooltip: {
                    ...
                    formatter() {
                        ...
                        (this as any).points?.forEach((p: any) => {
                            if (p.y > 0) {
                                total += p.y;
                                html += `<span style="color:${p.color}">●</span> ${p.series.name}: <b>${fmt(p.y, currency)}</b><br/>`;
                            }
                        });
                        html += `<hr style="border:none;border-top:1px solid #e5e7eb;margin:6px 0"/><b>Total: ${fmt(total, currency)}</b>`;
                        return html;
    ...
    // ── ECHARTS — matching ExpenseTreeChart and MonthlyRadarChart ──────────────
    function renderECharts() {
        ...
        // ── RADAR — exact MonthlyRadarChart style ──
        // MonthlyRadarChart: Credit=#60d394 fillOpacity=0.4, Debit=#ef4444 fillOpacity=0.4
        // PolarGrid strokeDasharray="4 4", outerRadius="75%"
        if (type === "radar") {
            ...
            chart.setOption({
                ...
                tooltip: {
                    ...
                    borderColor: "#e5e7eb",
                    borderRadius: 8,
                    // Matching RadarCustomTooltip from MonthlyRadarChart
                ...
                legend: {
                    bottom: 0,
                    data: datasets.map((d, i) => d.label || (i === 0 ? "Credit" : "Debit")),
                    textStyle: { fontSize: 12, color: "#374151" },
                    icon: "circle",
                },
                radar: {
                    indicator: labels.map(l => ({ name: l, max: maxVal })),
                    radius: "75%",       // matches MonthlyRadarChart outerRadius="75%"
                    center: ["50%", "48%"],
                    // Matching PolarGrid strokeDasharray="4 4"
                ...
                series: [{
                    ...
                    data: datasets.map((ds, i) => {
                        // MonthlyRadarChart: Credit stroke=#60d394 fill=#60d394 fillOpacity=0.4
                        //                   Debit stroke=#ef4444 fill=#ef4444 fillOpacity=0.4
                        const color = i === 0 ? RADAR_CREDIT : RADAR_DEBIT;
                        return {
        ...
        // ── TREE — exact ExpenseTreeChart style ──
        // ExpenseTreeChart: orient="LR", root symbol="diamond" color="#e36414"
        // parent-of-root symbol="rect" color="#0077b6"
        // level-1 symbol="circle" color="#6a994e"
        // leaves symbol="diamond" color="#02c39a"
        if (type === "tree") {
            const { nodes = [], totalDebit = 0 } = chartData;
            const branches = nodes.filter(n => n.parent === "root");

            // Build children matching ExpenseTreeChart buildTree function
            const children = branches.map((n, i) => {
                // Proportional symbol size like ExpenseTreeChart
                ...
                return {
                    ...
                    value: n.value,
                    symbolSize: symSize,
                    symbol: "circle",           // level-1 children = circle (from buildTree)
                    itemStyle: {
                        color: TREE_COLORS.circle, // #6a994e
                        borderColor: TREE_COLORS.circle,
                        borderWidth: 1,
                    },
                    label: {
                        show: true,
            ...
            // Root node — diamond, #e36414 (ExpenseTreeChart root color)
            const treeData = [{
                name: "All\nExpenses",
                value: totalDebit,
                symbolSize: fullscreen ? 30 : 24,
                symbol: "diamond",              // root = diamond (from buildTree)
                itemStyle: {
                    color: TREE_COLORS.root,    // "#e36414"
                    borderColor: TREE_COLORS.root,
                    borderWidth: 2,
                },
                label: {
                    show: true,
            ...
            chart.setOption({
                ...
                tooltip: {
                    trigger: "item",
                    // Matching ExpenseTreeChart tooltip formatter
                    triggerOn: "mousemove",
                ...
                series: [{
                    ...
                    data: treeData,
                    layout: "orthogonal",
                    orient: "LR",           // ExpenseTreeChart orient: "LR"
                    top: "1%",
                    left: "15%",            // ExpenseTreeChart left: "15%"
                    bottom: "1%",
                    right: rightMargin,
                    symbolSize: 14,         // ExpenseTreeChart symbolSize: 14
                    lineStyle: { color: "#d1d5db", width: 1.5 }, // ExpenseTreeChart lineStyle
                    label: { show: false },
                    leaves: { label: { show: false } },
                    expandAndCollapse: true,    // ExpenseTreeChart expandAndCollapse: true
                    emphasis: { focus: "descendant" }, // ExpenseTreeChart emphasis
                    initialTreeDepth: 2,        // ExpenseTreeChart initialTreeDepth: 2
                    animationDuration: 350,     // ExpenseTreeChart animationDuration: 350
                    animationDurationUpdate: 350,
                    nodePadding: 25,            // ExpenseTreeChart nodePadding: 25
                }],
            });

            // ResizeObserver like ExpenseTreeChart
            try {
    ...
    return (
        <div style={{ background: "#ffffff", overflow: "hidden" }}>
            {/* Header — matching your existing card headers */}
            <div style={{
            ...
            {/* Legend for bar/line/area (non-pie, non-radar which have built-in legends) */}
            ...
            {/* YearlyCreditDebitChart-style bottom legend for daywise */}
            {type === "daywise" && (
                <div style={{ display: "flex", justifyContent: "center", gap: "16px", padding: "6px 14px 0", fontSize: "12px" }}>
                    <span style={{ display: "flex", alignItems: "center", gap: "5px" }}>
                        <span style={{ width: 10, height: 10, borderRadius: "50%", background: DAYWISE_CREDIT, display: "inline-block" }} />
                        Credit
                    </span>
                    <span style={{ display: "flex", alignItems: "center", gap: "5px" }}>
                        <span style={{ width: 10, height: 10, borderRadius: "50%", background: DAYWISE_DEBIT, display: "inline-block" }} />
                        Debit
                    </span>
                </div>
            )}
            ...
            <div style={{ padding: fullscreen ? "10px 14px 14px" : "8px 10px 10px" }}>
                {!ready ? (
                    // Loading state matching your existing animate-pulse style
